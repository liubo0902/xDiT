...
def xdit_ring_sage_attn_forward(
    process_group,
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    for step in range(comm.world_size):
        ...
        if not causal or step <= comm.rank:
            ...
            block_out, block_lse = sageattn(q, key, value, is_causal=False, return_lse=True)
    ...
    out = out.to(q.dtype).transpose(1, 2)
    if attn_type != AttnType.SPARSE_SAGE:
        lse = lse.squeeze(dim=-1).transpose(1, 2)
    return out, lse


class xFuserRingSageAttnFunc(RingFlashAttnFunc):
    @staticmethod
    def forward(
        ctx,
        q,
        ...
        if attn_layer is None:
            k = k.contiguous()
            v = v.contiguous()
        out, softmax_lse = xdit_ring_sage_attn_forward(
            group,
            q,
            k,
            v,
...
def xdit_ring_sage_attn_func(
    ...
    return xFuserRingSageAttnFunc.apply(
        q,
        k,
        v,
        dropout_p,
